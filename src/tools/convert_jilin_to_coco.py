...
import shutil
import cv2
from io import StringIO  # 新增：用于模拟文件对象，统一分隔符
HAVE_DET_FILE = False
HALF_VIDEO = False
...
# 新增：处理混合空格/逗号分隔符的GT文件读取函数
def load_mixed_delimiter_gt(ann_path):
    unified_lines = []
    with open(ann_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if not line:  # 跳过空行
                continue
            unified_line = line.replace(' ', ',')  # 空格→逗号，统一分隔符
            unified_lines.append(unified_line)
    # 用StringIO模拟文件，让np.loadtxt能按逗号解析
    return np.loadtxt(StringIO('\n'.join(unified_lines)), dtype=np.float32, delimiter=',')
def convert_airmot(data_p = '/workspace/AIR-MOT/', SPLITS=None):#'/workspace/JLTrack/'
    if SPLITS is None:
        SPLITS = ['train', 'test']
    OUT_PATH = os.path.join(data_p, 'annotations')#data_p + 'annotations/'
    if not os.path.exists(OUT_PATH):
        os.mkdir(OUT_PATH)
    for split in SPLITS:
        #######ata_path = data_p + (split if not HALF_VIDEO else 'train')
        ...
        for seq in sorted(seqs):
            ...
            if anns.size == 0:
                print(f'{seq} has no annotations!')
                continue
            ######
...
def convert_viso(root_path='/workspace/VISO/', SPLITS=None):
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            #anns = np.loadtxt(ann_path, dtype=np.float32, delimiter=',')
            anns = load_mixed_delimiter_gt(ann_path)  # 替换成新函数，自动处理混合分隔符
            youxiao_ann = 0
...
if __name__ == '__main__':
    ...
    convert_viso(root_path='/home/fh/fh_local/CFTracker-main/workspace/VISO-SPLIT/', SPLITS=['train','test'])#'/workspace/viso1st/'
    #convert_airmot(data_p='/home/fh/fh_local/CFTracker-main/workspace/AIR-MOT-SPLIT/', SPLITS=['train','test'])