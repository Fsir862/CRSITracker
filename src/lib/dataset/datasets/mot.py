...
class MOT(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        if opt.dataset_version in ['17trainval', '17test']:
            ...
            # 当 dataset_version 不是预期的值时，抛出错误
            raise ValueError("Unknown dataset version: {}".format(opt.dataset_version))

        img_dir = os.path.join(data_dir, 'test' if 'test' in self.dataset_version else 'train')
