...
class Tracker(object):
    ...
    def init_track(self, results):
        for item in results:
            if item['score'] > self.opt.new_thresh and item['class'] in [1,2]:
                self.id_count += 1
                item['active'] = 1
                item['age'] = 1
    ...
    def step(self, results, public_det=None):
        ret = []
        for i in range(1,3):
            ret = ret + self.step_oneclass(results, i)
        
        self.tracks = ret
        return ret

    def step_oneclass(self, results, classid, public_det=None):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        if cnt_box == 0:
            return []
        scores = np.array([item['score'] for item in results if item['class'] == classid], np.float32)
        bboxes = np.vstack([item['bbox'] for item in results if item['class'] == classid])  # N x 4, x1y1x2y2
