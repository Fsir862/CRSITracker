...
class Tracker(object):
    def __init__(self, opt):
        ...
        for c in range(self.opt.num_classes):
            self.mot_tracker.append(SingleTracker(self.opt))
    ...
    def step(self, results, public_det=None):
        ...
        for i in range(len(results)):
            dets_mc[results[i]["class"]].append(results[i])

        for c in range(self.opt.num_classes):
            STrack.shared_kalman = self.kalman[c]
            dets = dets_mc[c+1]
            rsts = self.mot_tracker[c].step(dets)
            for item in rsts:
                item['class'] = c+1
                ret.append(item)
        self.tracks = ret
