...
class Tracker(object):
    ...
    def init_track(self, results):
        for item in results:
            if item['score'] > self.opt.new_thresh:
                ...
                item['active'] = 1#连续匹配次数
                item['age'] = 1#未匹配到的时间
                item['tracking_id'] = self.id_count
                if not ('ct' in item):
                    bbox = item['bbox']
                    item['ct'] = [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2]#ct:当前帧中心
    ...
    def step(self, results, public_det=None):
        ...
        dets = np.array(
            [det['ct'] + det['tracking'] for det in results], np.float32)  # N x 2   tracking：位移向量，即网络预测的中心位移
        ...
        # ===== NEW: matching summary for logging =====
        try:
            num_matches = int(matches.shape[0]) if hasattr(matches, "shape") else int(len(matches))
            if num_matches > 0:
                md = [dist0[m[0], m[1]] for m in matches]
                mean_match_dist = float(np.mean(md))
            else:
                mean_match_dist = -1.0
            self.last_debug = {
                "trk_M_prev": int(M),
                "trk_N_det": int(N),
                "trk_num_matches": num_matches,
                "trk_unmatched_tracks": int(len(unmatched_tracks)),
                "trk_unmatched_dets": int(len(unmatched_dets)),
                "trk_mean_match_dist": mean_match_dist,
            }
        except Exception:
            self.last_debug = {}
        ...
        for m in matches:
            ...
            if self.opt.give_first_gt:
                track['class'] = self.tracks[m[1]]['class']
            track['age'] = 1
