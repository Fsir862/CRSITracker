...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--dla_node', default='dcn')  # dcn, gcn, conv
        ...
        # train
        self.parser.add_argument('--optim', default='adam')  # adam, sgd
        self.parser.add_argument('--lr', type=float, default=0.3125e-4,
                                 help='learning rate 1.25e-4 for batch size 32.')
        self.parser.add_argument('--lr_step', type=str, default='60,90',
                                 help='drop learning rate by 10.')
        self.parser.add_argument('--save_point', type=str, default='80,90',
        ...
        self.parser.add_argument('--K', type=int, default=400,
        ...
        self.parser.add_argument('--match_thresh', type=float, default=0.8)     # byte
        self.parser.add_argument('--track_buffer', type=int, default=30)        # byte
        self.parser.add_argument('--new_thresh', type=float, default=0)
        self.parser.add_argument('--max_frame_dist', type=int, default=10)  # 3
        ...
        self.parser.add_argument('--bird_view_world_size', type=int, default=64)  # byte
        ...
        self.parser.add_argument('--only_show_dots', action='store_true')  # must use with --show_track_color
        self.parser.add_argument('--show_trace', action='store_true')
        self.parser.add_argument('--show_arrow', action='store_true')


        self.parser.add_argument('--demo_videos', action='store_true', help='demo many videos and save txt')
        self.parser.add_argument('--nms', action='store_true', help='run nms in testing.')
        self.parser.add_argument('--nms_thresh', type=float, default=0.4, help='')
        self.parser.add_argument('--auto_thresh', action='store_true', help='')
        self.parser.add_argument('--leiji_test', action='store_true')
        self.parser.add_argument('--track_method', type=str, default='src')  # src, byte, sort
        self.parser.add_argument('--pre_hm_method', type=str, default='add')  # add, concat, no
        self.parser.add_argument('--atten_method', type=str, default='none')  # reweight, corr
        self.parser.add_argument('--nms_type', type=str, default='maxpool')  # maxpool, def_conv, none
        self.parser.add_argument('--shortnet', action='store_true')
        self.parser.add_argument('--lowfeat', action='store_true')
        self.parser.add_argument('--mtl_loss', action='store_true')
        self.parser.add_argument('--freeze_encoder', action='store_true')
        self.parser.add_argument('--no_prehm_input', action='store_true')
        self.parser.add_argument('--give_first_gt', action='store_true')
        self.parser.add_argument('--give_sot_gt', action='store_true')
        self.parser.add_argument('--big_radius', action='store_true', help="max(radius, min(h,w)/2)  !!别用")
        self.parser.add_argument('--mask_input', action='store_true', help="use vibe mask for motion object detection") # TODO
        self.parser.add_argument('--class_balance', type=float, default=0, help="use ClassBalancedDataset")
        self.parser.add_argument('--inference_train', action='store_true')
        # add to your argument parser (opts.py)
        self.parser.add_argument('--mask_enable', action='store_true', default=False,
                                help='enable masked channel attention (in model)')
        self.parser.add_argument('--mask_sigma', type=float, default=1.5, help='gaussian sigma for mask smoothing')
        self.parser.add_argument('--mask_ksize', type=int, default=13, help='gaussian kernel size (odd)')#11
        self.parser.add_argument('--mask_reduction', type=int, default=16, help='reduction ratio for channel MLP in mask attention')
        self.parser.add_argument('--mask_detach', action='store_true', help='detach heatmap mask (default True if not set)')
        self.parser.add_argument('--mask_debug', action='store_true', default=False,
                                help='if set, print/save mask debug info during training/inference')
        self.parser.add_argument('--mask_adaptive', action='store_true', default=False,
            help='Use bbox-adaptive anisotropic Gaussian mask.')
        self.parser.add_argument('--mask_sigma_scale', type=float, default=0.6,#0.6
            help='sigma_x = scale * (bbox_w/2), sigma_y = scale * (bbox_h/2) in heatmap coords.')
        self.parser.add_argument('--mask_sigma_min', type=float, default=1.0,
            help='minimum sigma in pixels on the heatmap.')
        self.parser.add_argument('--mask_sigma_max', type=float, default=8.0,#8.0
        ...
        self.parser.add_argument('--upm_search', type=int, default=11)
        self.parser.add_argument('--upm_s_default', type=float, default=3.0)
        self.parser.add_argument('--upm_ncc', type=int, default=1) 
        self.parser.add_argument('--upm_detach_feat', type=int, default=0)  
        self.parser.add_argument('--gat', action='store_true', help='enable graph attention fusion')
        self.parser.add_argument('--gat_dim', type=int, default=128)
        self.parser.add_argument('--gat_alpha', type=float, default=0.2)#1.5
        self.parser.add_argument('--gat_beta', type=float, default=1.0)
        self.parser.add_argument('--gat_inject', type=str, default='gauss', choices=['gauss','dconv','film'])
        self.parser.add_argument('--gat_k', type=int, default=3)
        self.parser.add_argument('--gat_pool', type=int, default=7)
        self.parser.add_argument('--gat_norm', type=int, default=1)
        self.parser.add_argument('--gat_detach_feat', type=int, default=1)
        self.parser.add_argument('--gat_inject_alpha_start', type=float, default=1.0)
        self.parser.add_argument('--gat_inject_alpha_end', type=float, default=1.0)
        self.parser.add_argument('--max_grad_norm', type=float, default=5.0)
        self.parser.add_argument('--gat_hm_center_loss_w', type=float, default=0.05)
        self.parser.add_argument('--gat_hm_bg_ratio', type=float, default=0.3)
        self.parser.add_argument('--gat_hm_bg_margin', type=float, default=0.02)

        self.parser.add_argument('--gauss_sigma_scale', type=float, default=1.0)
        self.parser.add_argument('--gauss_sigma_min', type=float, default=0.5)
        self.parser.add_argument('--gauss_sigma_max', type=float, default=2.5)
        self.parser.add_argument('--gauss_rad_min', type=int, default=1)
        self.parser.add_argument('--gauss_rad_max', type=int, default=5)
        self.parser.add_argument('--gauss_k_sigma', type=float, default=3.0)
        self.parser.add_argument(
            '--gat_size_factor_min', type=float, default=0.01,
            help='per-node minimal scale for large targets in GAT injection (0~1).'
        )
        self.parser.add_argument(
            '--gat_size_factor_max', type=float, default=1.0,
            help='per-node maximal scale for small targets in GAT injection (usually 1.0).'
        )
        self.parser.add_argument(
            '--gat_size_gamma', type=float, default=1.5,
            help='shape exponent for size-based attenuation (>1 makes large targets decay faster).'
        )
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        # 先根据权重过滤 heads
        for head in list(opt.weights.keys()):
            if opt.weights[head] == 0 and head in opt.heads:
                del opt.heads[head]
        # 再补上 upm 的权重（它本来就不是一个 head，只是 loss 统计项）
        opt.weights['upm'] = opt.upm_loss_w
