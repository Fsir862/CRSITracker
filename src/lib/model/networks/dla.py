...
from ..masked_channel_attention import MaskedChannelAttention
    ...
    def __init__(self, num_layers, heads, head_convs, opt):
        ...
        if getattr(self.opt, 'mask_enable', False):
            ...
            self.mask_att = MaskedChannelAttention(
                channels=feat_ch,
                reduction=getattr(self.opt, 'mask_reduction', 16),
                ksize=getattr(self.opt, 'mask_ksize', 7),
                ...
                soft_radius=getattr(self.opt, 'mask_soft_radius', 2.0),
            )
        else:
            self.mask_att = None
        ...
        if self.opt.atten_method != "none":
            if self.opt.mode == "train":
                ...
                if self.heat_att is None:
                    self.heat_att = torch.full([y[-1].shape[0], self.heads['hm'], y[-1].shape[2], y[-1].shape[3]], 0.7).to(y[-1].device)
                if self.offset is None:
                    self.offset = torch.zeros([y[-1].shape[0], 2, y[-1].shape[2], y[-1].shape[3]]).to(y[-1].device)
                ...
                self.last_heat_att = self.heat_att.detach() if isinstance(self.heat_att, torch.Tensor) else None
            if getattr(self, 'mask_att', None) is not None:
                try:
                    ret, Mc_gauss  = self.mask_att(ret, self.heat_att)
                except Exception as e:
                    if getattr(self.opt, 'mask_debug', False):
                        print(f"[mask_att failed] {type(e).__name__}: {e}", flush=True)
            ...
            prev_feat = None
            try:
                if getattr(self.opt, 'upm', False) and (pre_img is not None):
                    ...
                    try:
                        if getattr(self.opt, 'upm', False) and (pre_img is not None):
                            ...
                            self.eval()
                            with torch.no_grad():
                                x_prev = self.base(pre_img, None, None)
                                x_prev = self.dla_up(x_prev)
                                y_prev = []
                                for i in range(self.last_level - self.first_level):
                                    y_prev.append(x_prev[i].clone())
                                self.ida_up(y_prev, 0, len(y_prev))
                                prev_feat = y_prev[-1]
                            if was_training:
                                self.train()
                    except Exception as e:
                ...
                prev_feat = None
            if getattr(self.opt, 'upm', False):
                self.last_feat_curr = ret.detach() if getattr(self.opt, 'upm_detach_feat', 1) else ret
                self.last_feat_prev = prev_feat.detach() if (prev_feat is not None and getattr(self.opt, 'upm_detach_feat', 1)) else prev_feat

                if getattr(self.opt, 'mask_debug', False):
                    if not hasattr(self, '_upm_flag_curr'):
                        print(f'[UPM] cache last_feat_curr: {tuple(self.last_feat_curr.shape)}', flush=True)
                        self._upm_flag_curr = True
                    if (self.last_feat_prev is not None) and (not hasattr(self, '_upm_flag_prev')):
                        print(f'[UPM] cache last_feat_prev: {tuple(self.last_feat_prev.shape)}', flush=True)
                        self._upm_flag_prev = True
            return [ret]
        return [y[-1]]