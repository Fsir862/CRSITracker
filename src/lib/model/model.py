...
import os
# ===== UPM imports (新增) =====
from .modules.upm.upm import UncertainDispUPM
from .modules.upm.config import UPMConfig
# =============================
...
def create_model(arch, head, head_conv, opt=None):
    ...
    if opt is not None and getattr(opt, 'upm', False):
        last_stride   = getattr(model, 'last_stride', getattr(opt, 'down_ratio', 4))
        model.upm_cfg = UPMConfig.from_opt(opt, last_stride=last_stride)
        model.upm     = UncertainDispUPM(model.upm_cfg)
    if opt is not None and getattr(opt, 'gat', False):
        from lib.model.modules.gat import GATConfig, GraphFuse
        last_stride   = getattr(model, 'last_stride', getattr(opt, 'down_ratio', 4))
        feat_channels = getattr(model, 'last_channel', getattr(model, 'feat_channels', 64))
        model.gat_cfg = GATConfig.from_opt(opt, last_stride=last_stride, feat_channels=feat_channels)
        model.gat     = GraphFuse(model.gat_cfg)
    return model
